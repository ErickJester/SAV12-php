...
<div class="card p-4 mb-4" style="border-radius:16px;border:none;box-shadow:0 4px 15px rgba(0,0,0,0.05)">
    ...
    <form action="/admin/categorias/crear" method="POST" class="row g-2">
        <?= Session::csrfField() ?>
...
<div class="card"><div class="table-responsive"><table class="table table-hover mb-0">
    ...
    <?php foreach ($categorias as $c): ?>
        ...
        <td><?php if ($c['activo']): ?><form action="/admin/categorias/<?= $c['id'] ?>/desactivar" method="POST" class="d-inline"><?= Session::csrfField() ?><button class="btn btn-sm btn-warning">Desactivar</button></form><?php endif; ?></td></tr>
    <?php endforeach; ?>
    </tbody></table></div></div>
