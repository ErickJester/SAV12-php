...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="<?= base_url('admin/tickets') ?>" class="btn btn-outline-secondary"><i class="bi bi-arrow-left"></i> Volver</a>
</div>
<div class="row">
    <div class="col-md-8">
        ...
        <div class="card p-4 mb-4" style="border-radius:16px;border:none;box-shadow:0 4px 15px rgba(0,0,0,0.05)">
            ...
            <div class="d-flex gap-2 flex-wrap">
                <?php if (empty($ticket['asignado_a_id'])): ?>
                    <form action="<?= base_url('admin/tickets/' . $ticket['id'] . '/asignarme') ?>" method="POST" class="d-inline"><?= Session::csrfField() ?><button class="btn btn-success"><i class="bi bi-person-plus"></i> Asignarme</button></form>
                <?php endif; ?>
                <?php if (empty($ticket['asignado_a_id'])): ?>
                    <form action="<?= base_url('admin/tickets/' . $ticket['id'] . '/asignar-tecnico') ?>" method="POST" class="d-inline-flex gap-1"><?= Session::csrfField() ?>
                ...
                <?php if (in_array($ticket['estado'], ['RESUELTO','CERRADO','CANCELADO'])): ?>
                    <form action="<?= base_url('admin/ticket/' . $ticket['id'] . '/reabrir') ?>" method="POST" class="d-inline"><?= Session::csrfField() ?><button class="btn btn-warning"><i class="bi bi-arrow-counterclockwise"></i> Reabrir</button></form>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="card p-4 mb-4" style="border-radius:16px;border:none;box-shadow:0 4px 15px rgba(0,0,0,0.05)">
            ...
            <?php endforeach; ?>
            <form action="<?= base_url('admin/ticket/' . $ticket['id'] . '/comentar') ?>" method="POST" class="mt-3">
                <?= Session::csrfField() ?>
