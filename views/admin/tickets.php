...
<div class="mb-3">
    <a href="<?= base_url('admin/tickets') ?>" class="btn btn-sm <?= !isset($_GET['filtro']) ? 'btn-primary' : 'btn-outline-primary' ?>">Todos</a>
    <a href="<?= base_url('admin/tickets') ?>?filtro=activo" class="btn btn-sm <?= ($_GET['filtro'] ?? '') === 'activo' ? 'btn-warning' : 'btn-outline-warning' ?>">Activos</a>
    <a href="<?= base_url('admin/tickets') ?>?filtro=resuelto" class="btn btn-sm <?= ($_GET['filtro'] ?? '') === 'resuelto' ? 'btn-success' : 'btn-outline-success' ?>">Resueltos</a>
</div>
<div class="card"><div class="table-responsive"><table class="table table-hover mb-0">
    ...
    <?php foreach ($tickets as $t): ?>
        <tr>
            <td><?= $t['id'] ?></td>
            <td><a href="<?= base_url('admin/ticket/' . $t['id']) ?>"><?= htmlspecialchars($t['titulo']) ?></a></td>
            ...
            <td>
                ...
                <form action="<?= base_url('admin/tickets/' . $t['id'] . '/asignar-tecnico') ?>" method="POST" class="d-inline-flex gap-1">
                    <?= Session::csrfField() ?>
